...
//  ESP32 et navigateur utilisent uniquement HTTP
//  Polling navigateur : toutes les 2s via /api/poll
// ================================================
...
// ESP32 lit les commandes (polling)
...
// ESP32 envoie feedback état vannes
...
// ── POLL principal navigateur — toutes les 2s ────
// Retourne tout l'état du système en 1 requête
app.get('/api/poll', async (req, res) => {
  try {
    ...
    const esp32Online = lastSeen && (Date.now() - lastSeen < 12000);
...
app.get('/api/status', async (req, res) => {
  try {
    const [status] = await q('SELECT * FROM hs_status ORDER BY id DESC LIMIT 1');
    const lastSeen = status?.last_seen || 0;
    res.json({ success: true, lastSeen, esp32Online: !!(lastSeen && Date.now() - lastSeen < 12000) });
  } catch(e) { res.status(500).json({ success: false, message: e.message }); }
});
...
initDB().then(() => {
  server.listen(PORT, '0.0.0.0', () => {
    console.log(`\n🌿 HydroSmart — HTTP PURE (pas de WebSocket)`);

...
def add_pointage():
    ...
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute(f"SELECT id FROM employees WHERE id = {PLACEHOLDER}", (data.get("employeeId"),))
        employee = cur.fetchone()
        if not employee:
            return jsonify({"success": False, "message": f"Employé avec ID {data.get('employeeId')} non trouvé"}), 404
...
def handle_rssi_data(data):
    ...
    try:
        ...
        if processed:
            socketio.emit("rssi_update", {"data": processed})
            logger.info(f"📶 WebSocket émis: {len(processed)} valeurs RSSI")
        emit("ack", {"success": True, "message": f"{len(badges)} mesures traitées"})
    except Exception as e:
        logger.error(f"❌ Erreur traitement RSSI WebSocket: {e}", exc_info=True)

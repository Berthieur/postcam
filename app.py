...
if NUMPY_AVAILABLE:
    logger.info("✅ NumPy et SciPy disponibles - Mode haute performance")
else:
...
class TriangulationConfig:
    """Configuration centralisée pour la triangulation"""
    
    # Fenêtre temporelle pour mesures RSSI (secondes)
    MEASUREMENT_WINDOW = 2.5  # Réduit à 2.5s pour ultra-réactivité
    
    # Modèle de propagation RSSI → Distance
    RSSI_TX_POWER = -55      # Puissance à 1m (calibrer sur site)
    RSSI_PATH_LOSS = 3.0     # Exposant perte (2.0=libre, 4.0=obstrué)
    RSSI_MIN = -95           # Signal minimum acceptable
    RSSI_MAX = -30           # Signal maximum acceptable
    MAX_DISTANCE = 10.0      # Distance max réaliste (mètres)
    
    # Seuils adaptatifs selon qualité signal
    EXCELLENT_RSSI = -60     # > -60 dBm = excellent
    GOOD_RSSI = -70          # > -70 dBm = bon
    ALPHA_EXCELLENT = 0.45   # Très réactif
    ALPHA_GOOD = 0.35        # Équilibré
    ALPHA_WEAK = 0.20        # Très stable
    THRESHOLD_EXCELLENT = 0.025  # 2.5cm
    THRESHOLD_GOOD = 0.06        # 6cm
    THRESHOLD_WEAK = 0.12        # 12cm
    
    # Zone de travail (mètres)
    ZONE_WIDTH = 6.0
    ZONE_HEIGHT = 5.0
    
    # Filtrage statistique
    OUTLIER_SIGMA = 1.8      # Écart-type pour outliers (1.8σ ≈ 93%)
    MIN_ANCHORS = 3          # Minimum d'ancres pour triangulation
    
    # Cache et performance
    CACHE_SIZE = 128         # Taille cache LRU
    BATCH_SIZE = 50          # Taille batch pour traitement
    
    # Filtre de Kalman simplifié
    PROCESS_NOISE = 0.01     # Bruit du processus
    MEASUREMENT_NOISE = 0.1  # Bruit de mesure

# Instance config globale
config = TriangulationConfig()

# ========== CACHE INTELLIGENT ==========

class PositionCache:
    """Cache LRU avec TTL pour positions calculées"""
    
    def __init__(self, maxsize: int = 100, ttl: int = 5):
        self.cache: Dict[str, Tuple[float, float, float]] = {}  # emp_id: (x, y, timestamp)
        self.maxsize = maxsize
        self.ttl = ttl  # Time-to-live en secondes
        self.access_order = deque()
    
    def get(self, emp_id: str) -> Optional[Tuple[float, float]]:
        """Récupère position si cache valide"""
        if emp_id in self.cache:
            x, y, timestamp = self.cache[emp_id]
            # Vérifier TTL
            if time.time() - timestamp < self.ttl:
                # Mettre à jour ordre d'accès
    ...
    def set(self, emp_id: str, x: float, y: float):
        """Enregistre position dans cache"""
    ...
    def invalidate(self, emp_id: str):
        """Invalide cache pour un employé"""
        if emp_id in self.cache:
            del self.cache[emp_id]
        if emp_id in self.access_order:
            self.access_order.remove(emp_id)

# Instance cache globale
position_cache = PositionCache(maxsize=config.CACHE_SIZE, ttl=3)
...
def rssi_to_distance(rssi: int, tx_power: int = None, n: float = None) -> float:
    """
    Convertit RSSI → Distance avec cache LRU
    
    Formule: d = 10^((TxPower - RSSI) / (10 * n))
    
    Args:
        rssi: Signal reçu en dBm
        tx_power: Puissance à 1m (défaut: config)
        n: Exposant propagation (défaut: config)
...
def filter_outliers_sigma(distances: List[float], sigma: float = None) -> List[float]:
    """
    Filtre outliers par écart-type (fallback si < 4 mesures)
    
    Args:
        distances: Liste de distances
        sigma: Nombre d'écarts-types (défaut: config)
...
def get_adaptive_params(avg_rssi: float) -> Tuple[float, float, str]:
    """
    Retourne paramètres adaptatifs selon qualité signal
    
    Args:
        avg_rssi: RSSI moyen en dBm
    
    Returns:
        Tuple (alpha, threshold, quality_label)
    """
    if avg_rssi > config.EXCELLENT_RSSI:
        return (config.ALPHA_EXCELLENT, 
                config.THRESHOLD_EXCELLENT, 
                "excellent")
    elif avg_rssi > config.GOOD_RSSI:
        return (config.ALPHA_GOOD, 
                config.THRESHOLD_GOOD, 
                "good")
    else:
        return (config.ALPHA_WEAK, 
                config.THRESHOLD_WEAK, 
                "weak")

def trilateration_weighted_lsq(anchors: List[Dict]) -> Tuple[float, float]:
    """
    Trilatération par moindres carrés pondérés (NumPy/SciPy)
    
    Optimisations:
    - Pondération sigmoïde par RSSI
    - Initialisation par centroïde pondéré
    - Contraintes strictes de zone
    - Limite d'itérations pour performance
    
    Args:
        anchors: Liste [{x, y, distance, rssi}, ...]
    
    Returns:
        Tuple (x, y) position optimisée
    """
    ...
    rssis = np.array([a.get('rssi', -75) for a in anchors])
    
    # Pondération sigmoïde avancée
    # w = 1 / (1 + exp((rssi - center) / steepness))
    weights = 1.0 / (1.0 + np.exp((rssis + 70) / 6))
    
    # Normaliser poids (somme = 1)
    weights = weights / np.sum(weights)
    
    def residuals_weighted(p, positions, distances, weights):
        """Résidus pondérés pour least_squares"""
        x, y = p
        predicted = np.sqrt((positions[:, 0] - x)**2 + (positions[:, 1] - y)**2)
        return (predicted - distances) * weights * 10  # Scale pour convergence
    
    # Initialisation intelligente (centroïde pondéré)
    x_init = np.sum(positions[:, 0] * weights)
    y_init = np.sum(positions[:, 1] * weights)
    
    # Vérifier init dans zone
    x_init = np.clip(x_init, 0, config.ZONE_WIDTH)
    y_init = np.clip(y_init, 0, config.ZONE_HEIGHT)
    
    # Optimisation Trust Region Reflective
    try:
        result = least_squares(
            ...
            method='trf',
            ftol=1e-4,      # Tolérance fonction
            xtol=1e-4,      # Tolérance solution
            max_nfev=30     # Max 30 évaluations
        )
        
        return round(float(result.x[0]), 3), round(float(result.x[1]), 3)
    
    except Exception as e:
        logger.warning(f"⚠️ LSQ échoué: {e}, fallback centroïde")
        return round(x_init, 3), round(y_init, 3)

def trilateration_geometric(anchors: List[Dict]) -> Tuple[float, float]:
    """
    Trilatération géométrique (fallback sans NumPy)
    
    Résout système 3 équations à 3 cercles intersectants
    
    Args:
        anchors: Liste [{x, y, distance}, ...]
    ...
    # Trier par distance (prendre 3 plus proches)
    ...
    # Vérifier dénominateur non-nul
    if abs(denom) < 1e-9:
        # Ancres colinéaires → retourner centroïde
        x = (x1 + x2 + x3) / 3
        y = (y1 + y2 + y3) / 3
    else:
        x = (C * E - B * F) / denom
...
def trilateration(anchors: List[Dict]) -> Tuple[float, float]:
    """
    Point d'entrée trilatération avec sélection automatique
    ...
    if NUMPY_AVAILABLE:
        try:
            ...
            logger.debug(f"LSQ échoué: {e}, fallback géométrique")
...
def kalman_filter_simple(new_pos: Tuple[float, float], 
    ...
    """
    Filtre de Kalman simplifié (1D pour chaque axe)
    
    Équations:
    - Prédiction: x_pred = x_prev
    - Gain Kalman: K = P / (P + R)
    - Correction: x = x_pred + K * (z - x_pred)
    - Mise à jour: P = (1 - K) * P
    
    Args:
        new_pos: Nouvelle mesure (x, y)
        old_pos: Position précédente (x, y)
        process_noise: Bruit processus Q
        measurement_noise: Bruit mesure R
...
def calculate_and_broadcast_positions(cursor):
    """
    Calcul ultra-optimisé des positions
    
    Améliorations v2:
    - Fenêtre 2.5s (au lieu de 3s)
    - Filtrage IQR + σ combiné
    - Trilatération LSQ pondérée
    ...
    Args:
        cursor: Curseur DB actif
    """
    start_time = time.time()
    
    # Fenêtre temporelle ultra-réactive
    ...
    if not measurements:
        logger.debug("Aucune mesure récente")
        return
    ...
    for emp_id, anchors in employee_data.items():
        ...
        # Moyenner par ancre + filtrer outliers
        ...
        for aid, data in anchor_averages.items():
            # Filtrage double: IQR si ≥4 mesures, sinon σ
        ...
        if old_pos_row:
            ...
            if old_x is not None and old_y is not None:
                ...
                # Filtre Kalman + Lissage exponentiel hybride
                # Kalman pour bruit, puis EMA pour stabilité
                ...
                # Lissage exponentiel sur sortie Kalman
                ...
                if distance_moved < threshold:
                    logger.debug(
                        f"🔒 {emp_id[:8]}: Δ={distance_moved:.3f}m < {threshold:.3f}m "
                        f"[{quality}] → maintenu"
                    )
                    continue
                
                logger.info(
                    f"📍 {emp_id[:8]}: ({pos_x:.3f}, {pos_y:.3f}) "
                    f"Δ={distance_moved:.3f}m RSSI={avg_rssi:.0f}dBm "
                    f"α={alpha:.2f} [{quality}]"
                )
            else:
                pos_x, pos_y = new_x, new_y
                logger.info(f"📍 {emp_id[:8]}: Init ({pos_x:.3f}, {pos_y:.3f})")
        else:
            pos_x, pos_y = new_x, new_y
            logger.info(f"📍 {emp_id[:8]}: Première pos ({pos_x:.3f}, {pos_y:.3f})")
    ...
    if positions_updated > 0:
        logger.info(
            f"✅ {positions_updated} position(s) calculée(s) en {elapsed:.1f}ms "
            f"({len(measurements)} mesures)"
        )
...
def get_active_employees():
    try:
        ...
        logger.error(f"get_active_employees error: {e}")
        return jsonify({"success": False, "message": str(e)}), 500
...
def add_salary():
    ...
    try:
        ...
        logger.error(f"add_salary error: {e}")
        return jsonify({"success": False, "message": str(e)}), 500
...
def get_salary_history():
    try:
        ...
        logger.error(f"get_salary_history error: {e}")
        return jsonify({"success": False, "message": str(e)}), 500
...
def receive_rssi_data_http():
    """Réception données RSSI depuis ESP32"""
    data = request.get_json(silent=True)
    ...
    try:
        ...
        logger.debug(f"📡 Ancre #{anchor_id}: {len(badges)} badge(s)")
        ...
        for badge in badges:
            ...
            if not employee:
                logger.debug(f"Employé '{employee_name}' inconnu")
                continue
        ...
        logger.error(f"receive_rssi_data error: {e}", exc_info=True)
        return jsonify({"success": False, "message": str(e)}), 500
...
def add_pointage():
    ...
    try:
        ...
        logger.error(f"add_pointage error: {e}")
        return jsonify({"success": False, "message": str(e)}), 500
...
def get_pointage_history():
    try:
        ...
        logger.error(f"get_pointage_history error: {e}")
        return jsonify({"success": False, "message": str(e)}), 500

@app.route("/api/pointages/recent", methods=["GET"])
def get_recent_pointages():
    """Dernier pointage (10 dernières secondes) pour LCD"""
    try:
        ...
        logger.error(f"get_recent_pointages error: {e}")
        return jsonify({"success": False, "message": str(e)}), 500
...
if __name__ == "__main__":
    ...
    logger.info(f"⚙️ Configuration: fenêtre={config.MEASUREMENT_WINDOW}s, zone={config.ZONE_WIDTH}x{config.ZONE_HEIGHT}m")

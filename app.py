...
def get_active_employees():
    try:
        ...
        return jsonify({"success": False, "message": str(e)}), 500
# === POST ajouter salaire (CORRIGÉ - Gestion ID unique) ===
@app.route("/api/salary", methods=["POST"])
def add_salary():
    data = request.get_json(silent=True)
    logger.info(f"📥 Données reçues: {data}")
    ...
    try:
        ...
        # ✅✅✅ CORRECTION PRINCIPALE : TOUJOURS GÉNÉRER UN NOUVEL UUID
        # Ne jamais utiliser l'ID envoyé par le client
        salary_id = str(uuid.uuid4())
        
        logger.info(f"🆔 Nouvel ID généré: {salary_id} (ancien ID client ignoré)")

        # ✅ INSERTION DIRECTE (pas de vérification d'existence)
        cur.execute(f"""
            INSERT INTO salaries (id, employee_id, employee_name, amount, hours_worked, type, period, date)
            VALUES ({PLACEHOLDER}, {PLACEHOLDER}, {PLACEHOLDER}, {PLACEHOLDER}, {PLACEHOLDER}, {PLACEHOLDER}, {PLACEHOLDER}, {PLACEHOLDER})
        """, [
        ...
        conn.commit()
        logger.info(f"✅ Salaire créé: ID={salary_id}, employee_id={employee_id}, amount={amount}, type={record_type}")
        ...
        return jsonify({
            ...
            "id": salary_id,  # ✅ Retourner le vrai ID serveur
            "employeeId": employee_id
        }), 201

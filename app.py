...
def handle_rssi_data(data):
    ...
    try:
        ...
        # Fetch updated employee positions
        cur.execute(f"""
            SELECT id, nom, prenom, type, is_active, created_at,
                   email, telephone, taux_horaire, frais_ecolage,
                   profession, date_naissance, lieu_naissance,
        ...
        # Broadcast updated positions to Android clients
        socketio.emit('positions', {'success': True, 'employees': employees}, namespace='/api/employees/active')
        logger.info(f"📤 Broadcast positions à {len(employees)} employés actifs")
        
        emit('response', {'success': True, 'message': f"{len(badges)} mesures traitées"})
        
    except Exception as e:
        logger.error(f"❌ Erreur WebSocket RSSI: {e}", exc_info=True)
        emit('response', {'success': False, 'message': str(e)})

@socketio.on('connect', namespace='/api/employees/active')
def handle_positions_connect():
    logger.info("📱 Client WebSocket connecté à /api/employees/active")
    join_room('positions-room')

@socketio.on('disconnect', namespace='/api/employees/active')
def handle_positions_disconnect():
    logger.info("📱 Client WebSocket déconnecté de /api/employees/active")
